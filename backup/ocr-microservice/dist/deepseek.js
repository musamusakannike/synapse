...
export async function callDeepSeek(messages, jsonMode = false) {
    if (!DEEPSEEK_API_KEY) {
    ...
    try {
        ...
        throw new Error(error.response?.data?.error?.message || error.message || "Failed to query DeepSeek API");
    }
}
