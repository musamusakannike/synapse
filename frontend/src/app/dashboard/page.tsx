...
export default function DashboardPage() {
  ...
  const statCards = [
    {
      ...
      sub: `${stats?.coursesCompleted ?? 0} completed`,
    },
    {
      label: "Lessons completed",
      value: stats?.lessonsCompleted ?? 0,
      sub: `${stats?.totalCourses ?? 0} courses`,
    },
    {
      label: "Avg quiz score",
      value: stats ? `${stats.avgScore}%` : "0%",
      sub: `${stats?.totalAttempts ?? 0} attempts`,
    },
    {
      label: "Day streak",
      value: stats?.streak ?? 0,
      sub: stats?.streak ? "Keep it up!" : "Study today to start",
    },
  ];
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-5xl">
      ...
      {!loading &&
        data &&
        (data.continueLearning.length > 0 || data.scoreTrend.length > 0) && (
              ...
              <div className="p-5 rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)]">
                ...
                <div className="flex items-center justify-between mt-2 text-[11px] text-[var(--text-muted)]">
                  ...
                  <span className="text-[var(--text-secondary)] font-medium">
                    Latest:{" "}
                    {data.scoreTrend[data.scoreTrend.length - 1].percent}%
                  </span>
                  <span>
                    {formatDate(
                      data.scoreTrend[data.scoreTrend.length - 1].date,
                    )}
                  </span>
                </div>
              </div>
