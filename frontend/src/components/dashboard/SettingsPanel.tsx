...
export default function SettingsPanel() {
  ...
  useEffect(() => {
    userApi
      .profile()
      .then((res) => {
        if (res.data.success && res.data.data?.settings) setSettings(res.data.data.settings);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6" id="settings">
      <div>
        <h2 className="mb-1 text-lg font-semibold text-[var(--ink-900)]">Settings</h2>
        <p className="text-sm text-[var(--text-muted)]">Manage your preferences and account</p>
      </div>

      <Card className="p-6">
        <h3 className="mb-4 text-base font-semibold text-[var(--ink-900)]">Notifications</h3>
        <div className="space-y-4">
          {(['emailNotifications', 'pushNotifications', 'weeklyProgress'] as const).map((key) => (
            <div key={key} className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <Bell className="size-5 shrink-0 text-[var(--text-muted)]" />
      ...
      <Card className="p-6">
        <h3 className="mb-4 text-base font-semibold text-[var(--ink-900)]">Preferences</h3>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-[var(--line)] pt-4">
          <div className="flex items-center gap-3">
            <Target className="size-5 shrink-0 text-[var(--text-muted)]" />
            <div>
      ...
      <Card className="p-6">
        <h3 className="mb-4 text-base font-semibold text-[var(--ink-900)]">Account</h3>
