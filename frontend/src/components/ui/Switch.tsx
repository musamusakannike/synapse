  ...
  return (
    <label className={`inline-flex items-center gap-2.5 font-[var(--font-body)] ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
      <span
        ...
        className={`relative h-6 w-10 shrink-0 rounded-full transition-colors duration-[var(--duration-fast)] ${
          checked ? 'bg-[var(--brand-violet)]' : 'bg-[var(--line-strong)]'
        }`}
      >
        <span
          className="absolute top-[3px] size-[18px] rounded-full bg-white shadow-[var(--shadow-xs)] transition-all duration-[var(--duration-fast)]"
