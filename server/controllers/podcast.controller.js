...
export const generatePodcastScript = asyncHandler(async (req, res) => {
  ...
  switch (podcastStyle.toLowerCase()) {
    case "casual":
      styleInstructions = "Use a casual, friendly, and conversational tone as if talking to a friend.";
      break;
    case "energetic":
      styleInstructions = "Use an energetic, enthusiastic, and upbeat tone with excitement about the topic.";
      break;
    case "educational":
      styleInstructions = "Use an educational, informative tone that breaks down complex topics clearly.";
      break;
    default:
      styleInstructions = "Use a professional, clear, and engaging tone suitable for a news podcast.";
  }
...
Generate only the podcast script text without any introductory text or formatting:`;
  ...
  res.status(200).json({ 
    ...
    estimatedDuration: Math.ceil(cleanedScript.split(' ').length / 150) + " minutes", // ~150 words per minute
  ...
  try {
    ...
    const request = {
      ...
      voice: { 
        ...
        name: voiceGender.toLowerCase() === 'male' ? 'en-US-Journey-D' : 'en-US-Journey-F'
      },
      audioConfig: { 
        audioEncoding: 'MP3',
...
Generate only the podcast script text without any introductory text or formatting:`;
  ...
  return {
    script: cleanedScript,
    wordCount: cleanedScript.split(' ').length,
    estimatedDuration: Math.ceil(cleanedScript.split(' ').length / 150) + " minutes"
  };
}
...
export const getPodcastHistory = asyncHandler(async (req, res) => {
  ...
  const podcasts = await Podcast.paginate(
    { user: req.user._id },
    options
  );
