    ...
    topics: [
        {
            ...
            contents: [
                ...
                },
                {
                    ...
                    content: '# Create a project directory\nmkdir my-first-repo\ncd my-first-repo\n\n# Initialize a new Git repository\ngit init\n\n# Inspect hidden files (notice the newly created .git directory)\nls -la',
                    language: 'bash',
                },
                {
                    type: 'text',
                    title: 'What is inside the .git directory?',
                    content: 'When you run `git init`, Git creates a hidden `.git` folder in your project directory. This folder contains all the metadata, objects database, pointers, and configuration that power Git. If you delete the `.git` folder, you remove all Git history and turn the folder back into a regular directory.',
                },
            ],
            flashcards: [
            ...
            contents: [
                {
                    ...
                    content: 'While **Git** is the local command-line tool managing version control, **GitHub** is a cloud-hosted platform that hosts Git repositories online, adding tools for pull requests, issue tracking, CI/CD pipelines, and code reviews.',
                },
                {
                    type: 'code',
                    title: 'Setting Up Authentication & Adding Remotes',
                    content: '# Generate a secure SSH key pair\nssh-keygen -t ed25519 -C "your.email@example.com"\n\n# Display public key to copy into GitHub Settings -> SSH Keys\ncat ~/.ssh/id_ed25519.pub\n\n# Link your local repo to GitHub remote\ngit remote add origin git@github.com:username/my-repo.git\n\n# Verify registered remotes\ngit remote -v',
                    language: 'bash',
                },
                {
                    type: 'code',
                    title: 'Pushing, Fetching, and Pulling',
                    content: '# Push local main branch to GitHub and set tracking upstream (-u)\ngit push -u origin main\n\n# Download remote changes WITHOUT merging\ngit fetch origin\n\n# Download AND merge remote changes into current branch\ngit pull origin main',
                    language: 'bash',
                },
                {
                    type: 'text',
                    title: 'Ignoring Files with .gitignore',
                    content: 'Never commit secrets (API keys, passwords), dependencies (`node_modules`), or build output folders (`dist/`, `.env`) to GitHub.\n\nCreate a `.gitignore` file in your root folder:\n```\n# Dependencies\nnode_modules/\n\n# Environment secrets\n.env\n.env.local\n\n# Build artifacts\ndist/\nbuild/\n\n# OS files\n.DS_Store\n```',
                },
                {
                    type: 'quiz',
            ...
            contents: [
                {
                    ...
                    content: 'Suppose you are in the middle of building a feature and your team needs an urgent hotfix on `main`. You don\'t want to commit incomplete broken code. `git stash` temporarily saves your uncommitted working changes on a stack so you can switch branches cleanly.',
                },
                {
                    type: 'code',
                    title: 'Git Stash Commands',
                    content: '# Save uncommitted working edits to stash stack\ngit stash -u   # (-u includes untracked files)\n\n# List stashed items\ngit stash list\n\n# Re-apply most recent stash and remove it from stack\ngit stash pop\n\n# Apply specific stash without removing from stack\ngit stash apply stash@{0}',
                    language: 'bash',
                },
                {
                    type: 'text',
                    title: 'Cleaning Commit History with Interactive Rebase',
                    content: 'Interactive rebase (`git rebase -i`) lets you rewrite, combine, reorder, or delete local commits before sharing your branch.\n\nKeywords:\n- `pick`: Use commit as is.\n- `reword`: Change commit message.\n- `squash`: Combine commit into previous commit.\n- `drop`: Delete commit.',
                },
                {
                    type: 'code',
                    title: 'Interactive Rebase & Cherry-Pick Examples',
                    content: '# Start interactive rebase for last 3 commits\ngit rebase -i HEAD~3\n\n# Cherry-pick a specific commit from another branch into current branch\ngit cherry-pick b7e41a9',
                    language: 'bash',
                },
                {
                    type: 'text',
                    title: 'Emergency Recovery with git reflog',
                    content: 'If you accidentally ran `git reset --hard` or deleted a branch by mistake, don\'t panic! `git reflog` tracks every movement of HEAD on your local machine. You can find lost commit hashes and recover them easily.',
                },
                {
                    type: 'code',
                    title: 'Recovering Lost Commits using Reflog',
                    content: '# View safety log of HEAD movements\ngit reflog\n\n# Restore repository back to state before accidental reset\ngit reset --hard HEAD@{1}',
                    language: 'bash',
                },
                {
                    type: 'quiz',
...
const run = async () => {
    try {
        ...
        let existingCourse = await course_model_1.default.findOne({
            $or: [
                { title: { $regex: /git/i } },
